...
import edu.knoldus.ProductMessage.{
  Balance,
  Cancel,
  Deposit,
  GetCostOfProduct,
...
class Inventory extends FSM[InventoryState, InventoryData] {
  ...
  when(Open) {
    case Event(_, InventoryData(_, _, productLeft)) if productLeft <= 0 =>
      ...
      goto(PoweredOff)
    case Event(Deposit(value), InventoryData(currentTxTotal, _, _))
        if (value + currentTxTotal) >= stateData.costOfProduct =>
  ...
  when(Ready) { case Event(Product, InventoryData(currentTxTotal, costOfProduct, productLeft)) =>
  ...
  when(PoweredOff) {
    ...
    case _ =>
      ...
      sender ! InventoryError(
        "Inventory is stop.  Please start inventory first with StartUpInventory"
      )
      stay()
  }
